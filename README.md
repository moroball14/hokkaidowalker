...
※12/31デプロイしました。ログインページの「テストユーザーでログイン」を押してからお使いください。  
...
## 工夫したポイント
1. ユーザーの目的に合わせてイベントを検索できます  
→ gemのransackを使用し日付・カテゴリ検索機能を実装しました。

2. 地図にイベント一覧を表示します  
→ GoogleMaps API を用いました。また、位置検索と条件検索を組み合わせることにより、ユーザーが旅行の予定に組み込めるイベントを探しやすくしました。

3. 郵便番号から住所の自動入力できます（ハイフンあり・なしどちらも対応）  
→ CDNを組み込みました。ユーザーのイベント登録時の負担を減らします。

4. 北海道のイベントのみ入力できます  
→ 緯度と経度で制限かけることにより実装しました。

5. 開発環境をDockerで構築しました  
→ 最近現場で使用されている技術と知り、キャッチアップに努めました。
